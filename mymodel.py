...
parser.add_argument('--savefile',
                    type=str,
                    default='./savefile/cifar10/steam/test/ResnetWithourBottleneck_new/valacc/resnet110_prune_and_train_2x_densityp0.2_eachp0.05')
...
class Cifar_VGG:
    def __init__(self):
        ...
        self.train_data, self.test_data, self.train_db, self.val_db = data_provider(
            self.dataset, args.data_path, self.batchsize)
    ...
    def run(self):
        ...
        def train(epoch):
            ...
            for batch_idx, (
                    inputs,
                    targets) in enumerate(self.train_db if epoch == self.
                                          stream_epoch else self.train_data):
        ...
        def test(epoch):
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(
                        self.val_db if epoch ==
                        self.stream_epoch else self.test_data):
                    inputs, targets = inputs.to(self.device), targets.to(
                        self.device)
                    outputs = self.net(inputs)
                    loss = self.criterion(outputs, targets)
